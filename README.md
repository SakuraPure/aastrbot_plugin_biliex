...
在 WebUI 插件配置页可调整：定时推送（固定间隔）与不定时推送（间隔在最小/最大值之间随机取值，更不易触发风控）两个独立开关（开启任意一个即可推送，同时开启时以不定时为准）、检测间隔、获取视频数、已推送记录上限、单次推送上限、总结视频数上限、总结语言、是否含封面图、是否撤回 Cookie 消息、接口超时、HTTP 代理等。
